...
test('Jobs calendar mode titles the page Calendar', () => {
  assert.match(journey, /function renderCalendar\s*\(/);
  assert.match(journey, /updateChrome\('calendar'\)/);
  assert.match(journey, /hub-page-title">Calendar</);
  assert.match(journey, /calendar:\s*\{ title: 'Calendar'/);
});
...
test('Jobs list is jobs-first with collapsed filters and summary rail', () => {
  ...
  assert.match(journey, /jos-jobs-rail--summary/);
  assert.match(journey, /jos-jobs-sum-kpi/);
