...
test('Projects dashboard does not repeat the page title under the chrome', () => {
  const dash = js.match(/return '<div class="pp-shell pp-dash">[\s\S]*?<\/header>'/);
  assert.ok(dash, 'expected Projects dashboard header markup');
  ...
  assert.match(dash[0], /\+ New Project/);
});

test('Projects chrome still owns the single title', () => {
  assert.match(js, /titleEl\.textContent = 'Projects'/);
