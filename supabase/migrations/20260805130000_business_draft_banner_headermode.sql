-- Adds banner_url + meta.headerMode support to patch_business_in_progress,
-- for inline hero-image editing. Real columns/fields already read by the
-- renderer (wsPageEl('ws-hero-media') only shows a photo when
-- S.headerMode==='banner' && S.bannerUrl -- both required, confirmed by
-- reading hubly.html directly before writing this) -- same pattern as
-- logo_url and meta.businessType before it: keep the real render-path
-- fields in sync, don't invent a new one.
