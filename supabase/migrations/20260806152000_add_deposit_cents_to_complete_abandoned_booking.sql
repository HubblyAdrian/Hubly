-- Extend complete_abandoned_booking with the real deposit_cents value, same
-- pattern as every other field here — carries the resumed booking's real
-- configured deposit amount (from calcBookingMoney() client-side, same as
-- the direct-insert path in submitBooking()) instead of leaving it null
-- for bookings that went through the abandoned-resume path specifically.
...
drop function if exists complete_abandoned_booking(uuid,text,text,text,text[],text,text,text,text,text,text);
