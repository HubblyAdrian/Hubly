/**
 * Milestone 1.5 · Epic 2 — Change Plan DSL + Engine
 *
 * Declarative desired-state plans. Builders (later epics) figure out how to reach state.
 * No SQL, React, DB updates, component mutations, or API calls here.
 */

import type {
  BuilderIntent,
  BuilderRisk,
  BuilderAffectedSystem,
} from "./hubly_brain_builder_intent.ts";

export const CHANGE_PLAN_VERSION = "1.0.0" as const;
export const CHANGE_PLAN_OWNER = "hubly_brain" as const;

export type BuilderTypeId =
  | "website_builder"
  | "booking"
  | "crm"
  | "workspace_builder"
...
export type ChangePlanEstimatedImpact = {
  trustPct?: number | null;
  conversionPct?: number | null;
  schedulingFlexibility?: "low" | "medium" | "high" | null;
  complexity?: "low" | "medium" | "high" | null;
  summary: string;
};

/** One declarative change — describes end state, not procedural steps. */
export type ChangePlanAction = {
  actionId: string;
  /** Human builder name, e.g. "Booking Builder" */
  builderOwner: string;
  builderType: BuilderTypeId;
  system: BuilderAffectedSystem;
  /** Dot path into desiredState, e.g. booking.minimum_notice.hours */
  path: string;
  /** Desired value at that path */
  desired: unknown;
  reason: string;
  risk: BuilderRisk;
  dependencies: string[];
...
/**
 * Declarative desired end state — builders reconcile current → desired later.
 * Never procedural “move A then B then save”.
 */
export type ChangePlanDesiredState = {
  website?: {
    premium_feel?: boolean;
    hero?: { headline_tone?: "premium" | "clear" | "friendly" };
    faq?: { enabled?: boolean };
    booking_section?: { visible?: boolean; position?: "prominent" | "default" };
    explain_arrival_windows?: boolean;
  };
  booking?: {
    minimum_notice?: { hours: number };
    arrival_window?: {
      enabled: boolean;
  ...
  packages?: {
    tiers?: Array<{ name: string; enabled?: boolean }>;
  };
  marketplace?: {
    radius_miles?: number;
  };
...
export type ChangePlanValidation = {
  ...
  checked: {
    ...
    validCapabilities: boolean;
    validPermissions: boolean;
    supportedBuilders: boolean;
  };
};

export type ChangePlan = {
  id: string;
  version: typeof CHANGE_PLAN_VERSION;
  /** Primary builder, or "multi" when several systems. */
  builderType: BuilderTypeId;
  intentId: string;
  title: string;
  description: string;
  affectedSystems: BuilderAffectedSystem[];
  requiredCapabilities: BuilderIntent["requiredCapabilities"];
  /** Declarative end state (SSOT for builders). */
  desiredState: ChangePlanDesiredState;
  /** Flattened action metadata for Mission Control / validation. */
  changes: ChangePlanAction[];
  dependencies: string[];
  risk: BuilderRisk;
  confidence: number;
  ...
  reasoning: Array<{ reason: string; evidence: string[]; confidence: number }>;
...
export type ChangePlanEngineResult = {
  changePlan: ChangePlan;
  fromIntentId: string;
};

const SUPPORTED_BUILDERS = new Set<BuilderTypeId>([
  "website_builder",
  "booking",
  "crm",
  "workspace_builder",
...
type DraftAction = Omit<ChangePlanAction, "actionId" | "missionControlReplayId">;

function draftActionsFromIntent(intent: BuilderIntent): {
  desiredState: ChangePlanDesiredState;
  actions: DraftAction[];
} {
  const desiredState: ChangePlanDesiredState = {};
  const actions: DraftAction[] = [];
  const req = intent.originalRequest.toLowerCase();
  const systems = new Set(intent.affectedSystems);

  const push = (a: DraftAction) => {
    actions.push(a);
    setPath(desiredState as Record<string, unknown>, a.path, a.desired);
  };
  ...
  if (
    systems.has("Automations") ||
    /prep instruction|automation|workflow|reminder|review request|ask for (a )?review|quote.*(follow|5 day)|rain|reschedule exterior|membership|friday.*(summary|report)|recurring customer|when someone books/.test(
      req,
    )
  ) {
    ...
    if (/prep instruction|prep (email|message)|ceramic.*prep|prep.*ceramic|after ceramic|when someone books.*ceramic|books a ceramic/.test(req)) {
      ...
      push({
        ...
        reason: multi
          ? "Multi-system ceramic booking workflow from one conversation."
          : "Send prep instructions after ceramic coating bookings.",
        risk: "medium",
        dependencies: [],
        estimatedImpact: {
          trustPct: 10,
          complexity: "medium",
          summary: multi
            ? "Booking, CRM, portal, review, and upsell from one ask."
            : "Automated prep instructions reduce day-of questions.",
        },
    ...
    // Fallback if Automations system flagged but no specific pattern matched
  ...
  // Fallback: at least one declarative action from required capabilities
  if (!actions.length) {
    for (const cap of intent.requiredCapabilities.slice(0, 3)) {
      ...
      push({
        ...
        system: (TOOL_SYSTEM(cap.toolId) as BuilderAffectedSystem) || "Business",
...
function TOOL_SYSTEM(toolId: string): BuilderAffectedSystem {
  const map: Record<string, BuilderAffectedSystem> = {
...
/** Validate plan before Preview Engine exists. */
export function validateChangePlan(plan: Omit<ChangePlan, "validation"> & { validation?: ChangePlanValidation }): ChangePlanValidation {
  const issues: string[] = [];
  ...
  const validPermissions = true; // Epic 2: propose-only; permissions enforced at apply later
  ...
  for (const a of plan.changes) {
    ...
    if (a.capabilityId === undefined) {
      validCapabilities = false;
      issues.push(`Action ${a.path} missing capability`);
    }
  }

  // Simple conflict: same_day allowed true + minimum_notice hours
  ...
  // Procedural ban — no SQL/React/API language in desired values
...
function aggregateImpact(actions: DraftAction[]): ChangePlanEstimatedImpact {
  ...
  const complexity =
    actions.some((a) => a.estimatedImpact.complexity === "high")
      ? "high"
      : actions.some((a) => a.estimatedImpact.complexity === "medium")
      ? "medium"
      : "low";
  return {
    trustPct: trust || null,
...
function maxRisk(actions: DraftAction[]): BuilderRisk {
...
export function generateChangePlan(
  ...
  const builderType: BuilderTypeId = builders.length > 1 ? "multi" : (builders[0] || "multi");
  ...
  const draftPlan = {
    ...
    rollbackStrategy: {
      mode: "restore_previous_desired_state" as const,
  ...
  const changePlan: ChangePlan = { ...draftPlan, validation };

  return { changePlan, fromIntentId: intent.intentId };
}

export const HublyChangePlanEngine = {
  version: CHANGE_PLAN_VERSION,
