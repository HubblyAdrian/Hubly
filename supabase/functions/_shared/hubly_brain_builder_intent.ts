...
function collectSignals(request: string): CategorySignal[] {
  ...
  if (
    /move .*above|jobs above|sidebar order|workspace|dashboard layout|put .+ above|hide revenue|hide marketing|never.*(use|look)|pin quick|calendar.*(home|landing)|make calendar|mobile workspace|phone workspace|what (do you think|should i)|focus mode|job day/.test(
      r,
    )
  ) {
    signals.push({
      ...
      goal: /hide/.test(r)
        ...
        : /pin/.test(r)
        ? "Pin Action"
        : /calendar|homepage|landing|home/.test(r)
        ? "Adaptive Homepage"
        : /mobile|phone/.test(r)
        ? "Mobile Workspace"
        : /recommend|what (do you think|should i)/.test(r)
        ? "Workspace Recommendations"
        : /focus|job day|sales day/.test(r)
        ? "Focus Mode"
        : "Navigation Order",
      risk: "low",
      defaultCaps: [{
  ...
  if (/portfolio|upload.*(photo|image)|these \d+ photos|gallery|instagram|before.?after|hero.*photo|weak photo|organize everything|visual timeline/.test(r)) {
    signals.push({
      ...
      goal: /hero/.test(r)
        ? "Hero Replacement"
        : /instagram|carousel/.test(r)
        ? "Social Content"
        : /before.?after/.test(r)
        ? "Before/After Pairs"
        : /timeline|evolved/.test(r)
        ? "Visual Timeline"
