...
import {
  ...
  type AutomationIntelligencePlan,
} from "./hubly_brain_automation_intelligence.ts";
import {
...
export type HublyThinkResult = {
  ...
  /** Milestone 1.5 · Epic 4 — Collaboration & Approval (not applied). */
  ...
  /** Milestone 1.5 · Epic 9 — Automation Intelligence plan (not applied). */
...
export async function think(req: HublyThinkRequest): Promise<HublyThinkResult> {
  ...
  const preview = changePlan
    ...
    : null;
  // Milestone 1.5 · Epic 4 — Collaboration opens with "What do you think?" (not applied)
  const collaboration = preview && changePlan
    ? startCollaboration(preview, changePlan, { missionControlReplayId: null }).session
    : null;
  // Milestone 1.5 · Epic 5 — Version that would be created (Git for business; not applied)
  ...
  const workspaceIntelligence = wantsWorkspaceIntelligence && changePlan
    ...
    : null;
  // Milestone 1.5 · Epic 9 — Automation Intelligence (operations manager via conversation)
  ...
  const automationIntelligence = wantsAutomationIntelligence && changePlan
    ? buildAutomationIntelligence({
      businessId: businessId || `biz_${Date.now().toString(36)}`,
      plan: changePlan,
      industry: memory.industry || memory.business?.industry ||
    ...
    : null;
  // Milestone 1.5 · Epic 10 — Media Intelligence (understand + publish proposals; not published)
  ...
  const mediaIntelligence = wantsMediaIntelligence && changePlan
    ? buildMediaIntelligence({
      businessId: businessId || `biz_${Date.now().toString(36)}`,
      plan: changePlan,
      industry: memory.industry || memory.business?.industry ||
