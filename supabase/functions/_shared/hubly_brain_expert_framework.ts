...
export const EXPERT_FRAMEWORK_VERSION = "1.0.0" as const;

/** Expert ids are open strings — new experts must not require Brain type changes. */
export type HublyExpertId = string;

export type HublyExpertCapability = {
  /** What this expert can do (routing keywords). */
  can: string[];
...
export type HublyExpertDefinition = {
  ...
  version: string;
  purpose: string;
  /** Concrete responsibilities this expert owns. */
  responsibilities: string[];
  inputs: string[];
  outputs: string[];
  requiredMemory: string[];
  /** Allowed tools — mirrors capability.tools (Capability Registry). */
  allowedTools: string[];
  /** Allowed actions — mirrors capability.actions. */
  allowedActions: string[];
  /** Capability Registry — Brain routes from this. */
  capability: HublyExpertCapability;
  /** Baseline confidence this expert typically reports (0–100). */
  confidence: {
    baseline: number;
  ...
  reasoning: {
    required: boolean;
    fields: Array<"reason" | "evidence" | "confidence" | "expectedImpact">;
  };
  /**
   * Lower runs earlier. Experience Director should be highest (runs last).
   * Brain sorts solely by this — no hardcoded pipeline arrays.
   */
  executionPriority: number;
  failureBehavior: "skip" | "ask" | "fallback_local";
  /** Expert ids that must run before this one when selected. */
  dependencies: HublyExpertId[];
  /**
   * Intents this expert participates in.
   * Use ["*"] or omit with alwaysInclude for guardians.
   */
  intents?: string[] | null;
  /** Always included on customer-facing think (Experience Director). */
...
export type HublyExpertStatus = "ok" | "failed" | "skipped" | "retried";

/**
 * Structured expert return — every expert must produce this shape.
 * Customers never see this object; Hubly Brain merges into one response.
 */
export type HublyExpertOutput = {
  expertId: HublyExpertId;
  /** Human name from registration (Experience Director, Research Expert, …). */
  expertName?: string;
  ok: boolean;
  /** Execution status for Brain orchestration / transcripts. */
  status?: HublyExpertStatus;
  /** Wall-clock execution time for this expert (ms). */
  executionTimeMs?: number;
  /** How many retries Brain performed before this result. */
  retries?: number;
  summary: string;
  /**
   * Structured output — Research Report / Business Strategy / Creative Plan /
   * Quality Report / Experience Review. Prefer `output`; `payload` kept for compat.
   */
  ...
  confidence: number;
  questions?: string[];
  /** Internal only — never surface to customers. */
  error?: string | null;
};
...
const DISCOVERY_LOG: Array<{ at: string; event: string; expertId?: string; detail?: string }> = [];

function logDiscovery(event: string, expertId?: string, detail?: string): void {
  const entry = { at: new Date().toISOString(), event, expertId, detail };
  DISCOVERY_LOG.push(entry);
  while (DISCOVERY_LOG.length > 200) DISCOVERY_LOG.shift();
  console.log("HublyBrain.expertFramework", entry);
}

function cloneDef(def: HublyExpertDefinition): HublyExpertDefinition {
  return {
    ...def,
...
function normalizeDef(def: HublyExpertDefinition): HublyExpertDefinition {
  const capability = {
    can: [...(def.capability?.can || [])],
    tools: [...(def.capability?.tools || def.allowedTools || [])],
    reads: [...(def.capability?.reads || [])] as HublyExpertCapability["reads"],
...
export function registerExpert(def: HublyExpertDefinition, handler: HublyExpertHandler): void {
  if (!def?.id) throw new Error("Expert registration requires id");
  if (!handler) throw new Error(`Expert ${def.id} requires a handler`);
...
export function unregisterExpert(id: string): boolean {
  ...
  if (ok) logDiscovery("unregister", id, "removed");
  else logDiscovery("unregister_miss", id, "not_registered");
  return ok;
}
...
export function isExpertRegistered(id: string): boolean {
  return REGISTRY.has(String(id));
}

/** Automatic discovery — returns all registered expert definitions. */
export function discoverExperts(): HublyExpertDefinition[] {
...
export function listExpertCapabilities(): Array<{
  id: HublyExpertId;
  capability: HublyExpertCapability;
  purpose: string;
  executionPriority: number;
...
export function orderExpertsByPriority(ids: string[]): HublyExpertId[] {
...
function expertMatchesIntent(def: HublyExpertDefinition, intent: string, request: string): boolean {
  if (def.alwaysInclude) return true;
  const intents = def.intents || [];
  if (intents.includes("*") || intents.includes(intent)) return true;
...
export function selectExpertsFromRegistry(opts: {
  ...
  if (opts.forced?.length) {
    opts.forced.forEach((id) => {
      if (getExpert(id)) selected.add(String(id));
    });
    // Guardians still apply
    for (const e of listExperts()) {
      if (e.alwaysInclude) selected.add(e.id);
    }
  } else {
    for (const e of listExperts()) {
      if (expertMatchesIntent(e, intent, request)) selected.add(e.id);
    }
...
export function sanitizeExpertError(err: unknown): string {
...
function finalizeExpertOutput(
  id: HublyExpertId,
  name: string,
  out: HublyExpertOutput,
  meta: { executionTimeMs: number; retries: number; status: HublyExpertStatus },
): HublyExpertOutput {
...
export async function runExpert(id: HublyExpertId, ctx: HublyExpertContext): Promise<HublyExpertOutput> {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    ...
    try {
      ...
      const status: HublyExpertStatus = attempt > 0 ? "retried" : "ok";
