 ...
 *
 * Change Plan → living Preview. Owner sees / interacts / compares — then decides.
 * Never applies, saves, mutates, or publishes. Approval Engine is Epic 4.
 *
 * Signature moment: progressive stages so the owner watches Hubly build.
 */

import type {
  ChangePlan,
  ChangePlanAction,
  ChangePlanDesiredState,
  ChangePlanEstimatedImpact,
  BuilderTypeId,
} from "./hubly_brain_change_plan.ts";
import type { BuilderRisk } from "./hubly_brain_builder_intent.ts";

export const PREVIEW_ENGINE_VERSION = "1.0.0" as const;
export const PREVIEW_ENGINE_OWNER = "hubly_brain" as const;
...
export type PreviewCompareMode = "slider" | "side_by_side" | "diff" | "before_after";
...
export type PreviewStageStatus = "pending" | "running" | "complete";
...
export type PreviewChangeExplain = {
  actionId: string;
  path: string;
  builderOwner: string;
...
export type PreviewSurfaceBlock = {
...
export type PreviewOption = {
  ...
  recommended: boolean;
  summary: string;
  surfaces: PreviewSurfaceBlock[];
};

export type PreviewConversationTurn = {
  turnId: string;
  role: "owner" | "hubly";
  message: string;
  at: string;
...
export type BuilderPreview = {
  id: string;
  version: typeof PREVIEW_ENGINE_VERSION;
  changePlanId: string;
  intentId: string;
  title: string;
  headline: string;
  /** "Here's what I built." — never "Change applied." */
  voice: "heres_what_i_built";
  originalRequest: string;
  primarySurface: PreviewSurface;
  surfaces: PreviewSurfaceBlock[];
  ...
  conversation: PreviewConversationTurn[];
  lifecycle: PreviewLifecycle;
  lifecycleLog: Array<{ at: string; state: PreviewLifecycle; note: string }>;
  risk: BuilderRisk;
  confidence: number;
  estimatedImpact: ChangePlanEstimatedImpact;
  /** Epic 3 invariants */
  applied: false;
  executed: false;
  published: false;
...
export type PreviewEngineResult = {
...
function surfaceFromSystem(system: string): PreviewSurface {
...
function surfaceFromBuilderType(t: BuilderTypeId): PreviewSurface {
  switch (t) {
...
function compareModeFor(surface: PreviewSurface): PreviewCompareMode {
...
function defaultBeforeForPath(path: string): unknown {
...
function humanBeforeLabel(path: string, before: unknown): Record<string, unknown> {
...
function humanAfterLabel(path: string, desired: unknown, action: ChangePlanAction): Record<string, unknown> {
  if (path.startsWith("booking.arrival_window") || path.includes("arrival_window")) {
    const aw = desired as { before_minutes?: number; after_minutes?: number; enabled?: boolean };
    return {
  ...
  if (path.includes("minimum_notice")) {
    const h = (desired as { hours?: number })?.hours ?? desired;
    return {
  ...
  if (path.startsWith("automations.")) {
    ...
    const first = flows[0] as { steps?: Array<{ type: string }> } | undefined;
    return {
...
function whyForAction(action: ChangePlanAction): string {
...
function buildExplain(action: ChangePlanAction): PreviewChangeExplain {
...
function groupActionsBySurface(plan: ChangePlan): Map<PreviewSurface, ChangePlanAction[]> {
  const map = new Map<PreviewSurface, ChangePlanAction[]>();
  for (const action of plan.changes) {
    const surface = surfaceFromSystem(action.system) !== "multi"
      ? surfaceFromSystem(action.system)
      : surfaceFromBuilderType(action.builderType);
...
function surfaceTitle(surface: PreviewSurface): string {
  switch (surface) {
    case "website":
...
function wantsMultipleOptions(plan: ChangePlan): boolean {
  const req = String(plan.originalRequest || "").toLowerCase();
  return (
    plan.builderType === "website_builder" &&
    (/premium|luxury|modern|minimal|feel|brand|homepage|website/.test(req))
  );
}

function buildWebsiteOptions(base: PreviewSurfaceBlock[]): PreviewOption[] {
  const website = base.find((b) => b.surface === "website") || base[0];
  if (!website) return [];

  const mk = (id: string, label: string, vibe: string, recommended: boolean, tweak: Record<string, unknown>): PreviewOption => ({
    id,
    label,
    vibe,
    recommended,
...
function buildProgressiveStages(plan: ChangePlan, surfaces: PreviewSurface[]): PreviewStage[] {
...
function progressiveLabel(surface: PreviewSurface): string {
  switch (surface) {
    case "website":
...
function cloneSurfaces(surfaces: PreviewSurfaceBlock[]): PreviewSurfaceBlock[] {
  return JSON.parse(JSON.stringify(surfaces)) as PreviewSurfaceBlock[];
}

/**
 * Generate a living preview from a declarative Change Plan.
 * Does not apply, save, mutate, or publish anything.
 */
export function generatePreview(
  ...
  if (surfaceKeys.length === 0) {
    // Empty plan still gets a shell preview (should not happen for valid plans)
  ...
  const primarySurface =
    surfaces.length > 1 ? "multi" : surfaces[0]?.surface || surfaceFromBuilderType(plan.builderType);
  ...
  const activeSurfaces =
    selectedOptionId
      ? options.find((o) => o.id === selectedOptionId)?.surfaces || surfaces
      : surfaces;

  const stages = buildProgressiveStages(plan, surfaceKeys);
  const now = new Date().toISOString();
  const version1: PreviewVersion = {
    version: 1,
    at: now,
...
/** Mark preview viewed (Mission Control memory). */
export function markPreviewViewed(preview: BuilderPreview): BuilderPreview {
...
/**
 * Live preview conversation — owner feedback updates the preview (still no apply).
 * Examples: "I like Option 2", "Make it darker", "Bigger buttons".
 */
export function applyPreviewConversationTurn(
  preview: BuilderPreview,
  ownerMessage: string,
): BuilderPreview {
  ...
  const ownerTurn: PreviewConversationTurn = {
    turnId,
    role: "owner",
    message: msg,
    at,
  ...
  const hublyTurn: PreviewConversationTurn = {
    turnId: uid("pturn"),
    role: "hubly",
    message: hublyReply,
    at,
  ...
  const versionEntry: PreviewVersion = {
    version: nextVersion,
    at,
    summary: hublyReply,
    selectedOptionId,
...
/** Desired-state snapshot helper for tests / Mission Control. */
export function previewDesiredState(plan: ChangePlan): ChangePlanDesiredState {
  return plan.desiredState;
}

export const HublyPreviewEngine = {
  version: PREVIEW_ENGINE_VERSION,
