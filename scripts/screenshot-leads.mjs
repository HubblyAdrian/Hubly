/**
 * Capture Leads screenshot for visual review.
 * Usage: node scripts/screenshot-leads.mjs
 */
...
await page.evaluate(() => {
  ...
  Object.assign(st, {
    ...
    ownerFirst: 'Adrian',
    pipeline: { manual: [], deleted: [], stages: {}, lostReasons: {}, edits: {} },
  });
  ...
  const leads = document.getElementById('v-leads');
  if (leads) {
    leads.classList.remove('hidden');
    leads.hidden = false;
    leads.style.cssText = 'display:block!important;visibility:visible!important;opacity:1!important;height:100vh;overflow:auto;';
  }
  if (typeof showPage === 'function') showPage('p-app');
  HublyJourneyOS.renderLeads();
});

await page.waitForSelector('.jos-ld-shell', { state: 'attached', timeout: 15000 });
await page.evaluate(() => {
  const app = document.getElementById('p-app');
  if (app) app.style.cssText = 'display:flex!important;visibility:visible!important;opacity:1!important;position:fixed;inset:0;z-index:9999;';
  const v = document.getElementById('v-leads');
  if (v) v.style.cssText = 'display:block!important;visibility:visible!important;opacity:1!important;height:100vh;';
});
await sleep(800);
const shot = path.join(OUT, 'leads-screenshot-exact.png');
await page.screenshot({ path: shot, fullPage: false });
const debug = await page.evaluate(() => ({
  cards: document.querySelectorAll('.jos-ld-card').length,
  tabs: document.querySelectorAll('.jos-ld-stab').length,
  hasJordan: !!(document.body.innerText || '').includes('Jordan Lee'),
  totalLabel: (document.querySelector('.jos-ld-inbox-head strong') || {}).textContent || '',
}));
