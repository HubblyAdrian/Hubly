...
check(
  ...
  /Booking Intelligence|Workspace Intelligence|Automation Intelligence|Media Intelligence|Chat OS|Hubly Chat|Epic [7-9]|Epic 10|Epic 11|Deployment|Business Deployment|Epic 12/i.test(`${snap.builderActions?.epic || ""} ${snap.builderActions?.note || ""}`),
);
check("MC available flag is boolean", typeof snap.builderActions?.available === "boolean");
check(
  "MC recent surfaces intelligence",
  ["booking_intelligence", "automation_intelligence", "workspace_intelligence", "media_intelligence", "chat_os", "deployment"].includes(
