/** Node mirror of hubly_brain_automation_intelligence.ts — Milestone 1.5 Epic 9 (esbuild). */


// supabase/functions/_shared/hubly_brain_automation_intelligence.ts
var AUTOMATION_INTELLIGENCE_VERSION = "1.0.0";
var AUTOMATION_INTELLIGENCE_OWNER = "hubly_brain";
var AUTOMATION_INTELLIGENCE_LABEL = "Automation Intelligence Builder";
function uid(prefix) {
...
function extractAutomationWorkflows(plan, opts) {
...
function workflowsFromAction(a, request, _industry) {
  ...
  return desired.map((d, i) => {
    ...
    return {
      ...
      title: String(d.name || a.reason || "Workflow"),
...
function stepLabel(type, config) {
  if (/wait/.test(type)) {
...
function inferOutcomeFromPath(path, request) {
...
function workflowPrepCeramic(request) {
  return {
...
function workflowReviewRequest(request) {
...
function workflowQuoteFollowup(request) {
  return {
...
function workflowWeatherReschedule(request) {
  return {
    id: "weather_exterior",
...
function workflowMembershipBilling(request) {
  return {
...
function workflowFridaySummary(request) {
  return {
    id: "friday_summary",
...
function workflowRecurringCustomers(request) {
  return {
...
function workflowMultiSystemCeramic(request) {
  return {
...
function buildWorkflowPreview(workflows) {
  const primary = workflows[0];
  const steps = primary ? primary.steps.map((s) => s.label) : ["Trigger", "Action"];
  const graph = [];
  for (let i = 0; i < steps.length - 1; i++) {
    graph.push({ from: steps[i], to: steps[i + 1], label: "then" });
  }
  return {
    headline: primary ? `Preview: ${primary.title}` : "Workflow preview",
    graph,
    steps,
    note: "Nothing has been activated \u2014 preview only until you approve."
  };
}
function simulateWorkflows(workflows) {
  const has = (o) => workflows.some((w) => w.outcomeId === o);
  const bookings = 112;
  return {
    horizonDays: 30,
...
function scoreAutomationHealth(workflows) {
  ...
  const customerExperience = clamp(
    70 + (workflows.some((w) => w.outcomeId === "prep_instructions") ? 10 : 0) + (workflows.some((w) => w.outcomeId === "review_request") ? 8 : 0) + (workflows.some((w) => w.outcomeId === "quote_followup") ? 6 : 0)
  );
...
function buildAutomationRecommendations(workflows, industry) {
  const recs = [];
  const has = (o) => workflows.some((w) => w.outcomeId === o);
...
function buildAutomationDiscovery(workflows) {
  const discoveries = [];
  const has = (o) => workflows.some((w) => w.outcomeId === o);
  if (!has("review_request")) {
    discoveries.push({
...
function buildAutomationIntelligence(opts) {
  ...
  return {
    ...
    timeline: [
      ...
      },
      {
        ...
        detail: `30-day sim \xB7 ~${Math.round(minutes / 60 * 10) / 10}h/mo saved`
      },
      {
        at: nowIso(),
        event: "health_scored",
...
var HublyAutomationIntelligence = {
  version: AUTOMATION_INTELLIGENCE_VERSION,
