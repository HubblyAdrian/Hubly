/**
 * Node mirror of hubly_brain_decision.ts — Section 9 behavioral proofs.
 */
export const DECISION_ENGINE_VERSION = '1.0.0';
export const DECISION_ENGINE_OWNER = 'hubly_brain';
...
function inferCustomerImpactKinds(expectedOutcome) {
...
function scoreEvidenceQuality(opts) {
...
function scoreBusinessAlignment(opts) {
  let score = 30;
...
function scoreCustomerImpact(opts, kinds) {
  let score = 55;
...
function scoreExperienceImpact(opts) {
  ...
  if (/rewrite|redesign/.test(rec)) score += 5;
...
export function routeDecision(input) {
  ...
  if (input.decisionScore >= 75 && input.evidenceQuality >= 60) return 'recommend';
...
export function assessDecision(opts) {
  ...
  obj.dimensions.requiresApproval = obj.requiresApproval;
...
export function isWhyDecisionQuestion(request) {
...
export function answerWhyFromDecision(request, opts = {}) {
...
export function decisionActionToConfidenceBand(action) {
...
export function clearDecisionStoreForTests() {
  STORE.clear();
  BIZ_INDEX.clear();
  KEY_INDEX.clear();
}
