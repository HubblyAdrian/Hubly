...
ok(html.includes('id="transform"') || html.includes('The transformation'), 'transformation before/after');
ok(html.includes('After one conversation'), 'after-conversation payoff');
ok(html.includes('One conversation.') && html.includes('Everything changes'), 'transform headline');
ok(html.includes('Starting from') && html.includes('scratch'), 'before card framing');
ok(html.includes('Ready to') && html.includes('grow'), 'after card framing');
ok(html.includes('Live website') && html.includes('Customers can book'), 'after outcomes list');
ok(html.includes('xform-benefits') || html.includes('From idea to live business'), 'transform benefit bar');
