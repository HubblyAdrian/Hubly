/**
 * Capture Pipeline screenshot for visual review.
 * Usage: node scripts/screenshot-pipeline.mjs
 */
...
await page.evaluate(() => {
  ...
  Object.assign(st, {
    _ceoDemo: true,
    biz: "Adrian's Lawn Services",
    businessName: "Adrian's Lawn Services",
    ...
    quotes: [],
    smartQuotes: [],
    pipeline: { manual: [], deleted: [], stages: {}, lostReasons: {}, edits: {}, tags: {} },
  });
  ...
  const pipe = document.getElementById('v-pipeline');
  if (pipe) {
    pipe.classList.remove('hidden');
    pipe.hidden = false;
    pipe.style.cssText = 'display:block!important;visibility:visible!important;opacity:1!important;height:100vh;overflow:auto;';
  }
  if (typeof showPage === 'function') showPage('p-app');
  HublyJourneyOS.renderPipeline();
});

await page.waitForSelector('.jos-pk-shell', { state: 'attached', timeout: 15000 });
await page.evaluate(() => {
  const app = document.getElementById('p-app');
  if (app) {
    app.classList.add('jos-pixel', 'jos-pipeline-mode');
    app.style.cssText = 'display:flex!important;visibility:visible!important;opacity:1!important;position:fixed;inset:0;z-index:9999;';
  }
  const v = document.getElementById('v-pipeline');
  if (v) v.style.cssText = 'display:block!important;visibility:visible!important;opacity:1!important;height:100vh;';
});
await sleep(900);
const shot = path.join(OUT, 'pipeline-screenshot-exact.png');
await page.screenshot({ path: shot, fullPage: false });
const debug = await page.evaluate(() => ({
  cards: document.querySelectorAll('.jos-pk-card').length,
  cols: document.querySelectorAll('.jos-pk-col').length,
  hasAlex: !!(document.body.innerText || '').includes('Alex Rivera'),
  has8197: !!(document.body.innerText || '').includes('8,197'),
  hasDealWs: !!(document.body.innerText || '').includes('Deal Workspace'),
  hasGreeting: !!(document.body.innerText || '').includes('grow your business'),
}));
