...
### Homepage positioning (locked — HARD FREEZE)

Brand why (signature):

> **Most software makes you learn how it works. Hubly learns how you work.**
...
- **AI / Hubly Brain**: Centralized intelligence in
  ...
  - **Connector architecture:** DomainConnector (contract only — registrar TBD) ·
    PaymentConnector (Stripe) · CalendarConnector (Google) · Email/SMS/Maps/Ads/Accounting stubs.
    Owner-facing: “Connection required” / Connections page — never fake success.
    Premature Cloudflare/Porkbun/Namecheap implementations intentionally not shipped.
  - **Business Launch** (domain → DNS → SSL → publish + indexing) replaces “domain purchase”
  - **Website Runtime production surfaces:** SEO · FAQ schema · booking · lead forms ·
    analytics hooks · editing surfaces · sitemap/robots foundation
  - **Next priority:** Finish Website Runtime completely → then Business Launch (choose registrar)
  - Stages: Build → Launch → Run → Grow → Optimize
  - Constitution: `docs/HUBLY_CONSTITUTION.md` (Production-First + Connectors)
...
─────────────────────            ─────────────────────           ─────────────────────
AI Concierge                     Hubly Marketplace               Marketplace Ops
AI Matching                      (eng: Provider Experience /     Overview
Booking Engine                    Marketplace Lite packaging)    Providers
Confirmed Booking                Dashboard · Bookings            Verification
                                 Messages · Services             Bookings
                                 Availability · Profile          Analytics
                                 Payouts                         Trust & Safety
                                                                 Provider 360
...
| Owns | Does |
|---|---|
| **Hubly Marketplace** (eng: Provider Experience / Marketplace Lite) | Receive bookings, manage services, availability, messaging, get paid. **Nothing else.** |
...
| **Marketplace Ops** | Marketplace quality, verification, trust, analytics, fraud, moderation, provider lifecycle. Never CRM. Never Marketplace provider UI. |

Boundary test for Marketplace: *“Does this help a provider receive and complete
...
- **Phase 6 — Service Engine** — **FROZEN**.
  ...
  wins. Dual-write removed — writers persist `service_catalog` only;
  `getCatalog` migrate-on-reads legacy until re-save.
