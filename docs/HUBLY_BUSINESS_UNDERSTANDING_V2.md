...
**Naming precision, per `docs/HUBLY_CONVERSATION_CONTEXT_MODEL.md` Section 7:** this document specifies **Business Understanding** — the schema instance for knowledge about the business itself. It has siblings, not variants: **Customer Understanding** (Website/Concierge — what's known about a customer and their job) and **Marketplace Understanding** (what's known about a consumer's job spec, already implemented as `JobUnderstanding`). All three plug into the same generic, engine-owned patch mechanism. A Concierge conversation both *loads* the business's Business Understanding as Ground Truth (read-only — the business is already fully known) and *accumulates* its own separate Customer Understanding about the visitor, at the same time. This document only details the Business Understanding schema; Customer Understanding is named wherever it's relevant below, not fully specified here.
...
**Entry Intent is not a separate concept — it's Patch Zero.** Per `docs/HUBLY_CONVERSATION_CONTEXT_MODEL.md` Section 7: there is no "Business Understanding" and, separately, an "Entry Intent object." There is Understanding — one generic, engine-owned patch mechanism — and Entry Intent is simply the name for supplying its first patch up front, by the entry point, before turn one, instead of waiting for the model to emit it turn by turn. A service page passing `{selectedService, serviceId, pricing, duration, addOns}` is doing exactly what the model does when it emits a patch; it's just supplying it early because the UI already knows it. No new mechanism, no new layer, no separate object living alongside Understanding — the existing one, seeded early.

**Which Understanding schema Entry Intent seeds depends on context.** On a website (Concierge context), Entry Intent seeds **Customer Understanding** — a sibling schema to the Business Understanding one this document details below, holding what's known about *this customer and their job*: Selected Service, Selected Package, Vehicle/Property, Address, Preferred Date, Preferred Time, Budget, Photos, Special Requests. This document doesn't fully specify that schema — per the standing instruction, this is architecture support, not implementation — but it's named here so the shape is clear: the same 8-section treatment this document gives Business Understanding would eventually apply to Customer Understanding too, as its own document, when that becomes the active work.
...
- **Entry Intent as a new input to the conversation contract** — today's `hubly-conversation` request shape accepts `{messages, businessId?, understanding?}`; Entry Intent needs a place to arrive (most likely as an initial Understanding patch supplied alongside the first request, per Section 2 above) — a contract change, not implemented here.
