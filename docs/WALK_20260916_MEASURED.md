...
### The slug and the public URL both moved, and nothing said so

- seq 8, `03:07:08Z`, Hubly: *"The address **apollow**.myhubly.app is reserved for you."*
- `businesses.slug` now: **`apollo-weeds`**
- rows holding slug `apollow`: **zero**. That address resolves to nothing.

So the public web address Hubly had reserved for him in writing changed under him ninety seconds
later, the old one died, and **no sentence mentions it.** `syncDraftAddress`
(`hubly-conversation/index.ts:1424`) re-reads the slug and swaps the new address into the response so
the pill and the preview follow it, and logs the move to the server console. It tells the owner
nothing. The call-site comment says *"if the address moved, the response says so"* — the response
*carries* it; nobody *says* it.

**Not fixed — read-only measurement, as instructed.** The shape of the fix is one sentence on the
turn where `syncDraftAddress` finds a move, and it is prohibition 4 and 6 territory, not a nicety:
an address given to a customer stops working.
...
### A job sits at a time nobody asked for — and this is stated with its caveat first

**What would make this wrong:** `jobs` has **no `updated_at`** (no-trace instance #4), so nothing
dates this value or attributes it. The by-hand job editor (`platform-home.html:5965`) writes the same
column through the same RPC, and Adrian may have set it himself after the walk. There is no calendar
writer — `google_calendar_events` holds **0 rows** for this business. If either of those explains it,
the finding is void.

With that said: **the driveway job reads `17:00` — 5:00 PM.** `check-day-is-reachable.mjs`, written
2026-09-15, records it as `14:00`. seq 37 reports it set to "3:00" (15:00). seq 40 asks for **4 PM**
(16:00). **No turn in the conversation asks for 17:00.**

A previous session read `17:00` as "the 4 PM seq 40 asked for" and built the walk's headline on it.
17:00 is 5 PM. The write may still have landed; what cannot be said is that it landed *correctly*.

**The mechanism by which a time nobody said could reach that row is in the code and is not
hypothetical:** `updateJob` grounds the **address** (`addressGrounded`) and the **price**
(`priceGrounded`) against the owner's own message, and passes **`scheduled_time` straight through**
(`hubly_capability_registry.ts:8115`). The one field where a wrong value sends a person to a customer
at the wrong hour is the one field nothing checks. `update_business_job` then casts it with
`::time` and no validation — and Postgres **throws** on `'4 pm'`, accepting only `'4:00 PM'` or
`'16:00'`, so a malformed time is an exception rather than a refusal.

**Not built.** A `timeGrounded` fails in the safe direction (a refusal asks; it does not write a
wrong time), but it changes which writes are refused and that is a ruling.
