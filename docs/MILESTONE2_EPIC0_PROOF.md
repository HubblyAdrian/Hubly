# Milestone 2 · Epic 0 — Hubly Experience Layer

**Status:** PASS  
**Checked:** 2026-07-24T16:40:28.200Z  
**Gate:** `npm run check:m2-epic0` → this file + `docs/MILESTONE2_EPIC0_PROOF.json`

> This Epic is about the customer's emotional experience, not infrastructure.
> Reuses Hubly Brain, Experience Director, Identity System, Builder Engine, and Chat OS.
> No new AI systems.

## Philosophy

The owner should never feel like they are using software.
They should feel like they hired an experienced operator who wants their business to succeed.

## Proven

| # | Requirement | Status |
|---|-------------|--------|
| 1 | Experience Language System (sole customer-copy path) | ✅ |
| 2 | Dynamic Greeting System | ✅ |
| 3 | Thinking language replaces loading | ✅ |
| 4 | Conversation rules enforced (max 3, confidence-gated) | ✅ |
| 5 | Celebration system (meaningful only) | ✅ |
| 6 | Coaching language | ✅ |
| 7 | Honest disagreement | ✅ |
| 8 | Conversational error recovery | ✅ |
| 9 | Empty states teach | ✅ |
| 10 | Intentional transitions | ✅ |
| 11 | Visual personality (Hubly navy + orange) | ✅ |
| 12 | Emotional timeline | ✅ |
| — | Founder acceptance tests 1–4 | ✅ |

## Emotional timeline

Start → curiosity → discovery → confidence → excitement → pride → momentum → trust → partnership

## Transition pipeline

conversation → thinking → building → preview → approval → deployment → celebration

## Sample transcript (Founder Test 1)

**Owner:** Hey Hubly
**Hubly:** Research on pressure washing: Homeowners hire pressure washing for curb appeal before selling, parties, or spring refresh — they fear damage and no-shows more than price alone. I'm…

**Owner:** I'm starting a pressure washing business
**Hubly:** Research on pressure washing: Homeowners hire pressure washing for curb appeal before selling, parties, or spring refresh — they fear damage and no-shows more than price alone. I'm…

**Owner:** How's business doing?
**Hubly:** You've got this. We don't have to fix everything today — just the next right move. Research on pressure washing: Homeowners hire pressure washing for curb appeal before selling, pa…

**Owner:** What would you do if this were your business?
**Hubly:** I hear you — and I'd gently push a different direction. Want the short why? Research on pressure washing: Homeowners hire pressure washing for curb appeal before selling, parties, …

**Owner:** Make my homepage more premium
**Hubly:** I want to research a bit more before I commit — the evidence isn't strong enough yet. One thing before I go further — what matters more to you here?

**Owner:** I don't like it — undo that
**Hubly:** You're right to call that out. I looked again — here's the better take. Research on pressure washing: Homeowners hire pressure washing for curb appeal before selling, parties, or s…

**Owner:** Continue where we left off
**Hubly:** We were working on your last project (moving the business forward). Ready to continue?

**Owner:** Which quotes need follow-up?
**Hubly:** Research on pressure washing: Homeowners hire pressure washing for curb appeal before selling, parties, or spring refresh — they fear damage and no-shows more than price alone. I'm…

## Visual personality

- Navy: `#141B2B`
- Orange: `#D9632D`
- Wordmark: `assets/hubly-wordmark.png`

## Stop

Do **not** begin Epic 1 (Welcome to Hubly) until Founder Approval.
