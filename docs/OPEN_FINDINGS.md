...
| 4 | **A job change** — `jobs` has **no `updated_at` column**, so a row that moved carries no record of when, or that it moved at all. Found 2026-09-16 while trying to tell a successful time-change from a failed one on Adrian's walk — the question could not be answered from the row | **the cheapest honest record: an `updated_at timestamptz` with a trigger.** One column, one trigger, no new table. It does not say *what* changed, but it answers "did this move, and when", which is the question that could not be answered | **open — and now blocking a real question.** The driveway job on `hubly-classic-fixture` reads `17:00`; no turn in the conversation asked for 17:00 (see `docs/WALK_20260916_MEASURED.md`). Whether the writer wrote a time nobody said, or Adrian moved it by hand afterwards, cannot be told apart without this column |
| 5 | **A turn rescued by the no-silence floor** — the floor called `hcAppendMessage` and never `hcPersist`, so a turn it saved left no assistant row. "He was told nothing" and "he was told something and we did not keep it" produced byte-identical rows, and the thread IS the record on reload. Found 2026-09-16 answering exactly that question about seq 38/39/40 of Adrian's walk | the sentence itself, stored | **closed 2026-09-16** — the floor persists; `check-no-silent-turn.mjs` legs 12–14, red-proofed |
| 6 | **A detected capture miss that is deliberately not spoken** — created by the fix for the seq 44/45 contradiction: the guard now yields to any turn that really changed something | a row saying the miss was seen and not said | **closed on arrival 2026-09-16** — `capture_miss_events.outcome` = `quiet`, so the fix did not open a new blind spot |
| 7 | **A persist batch that never flushes** — `hcFlushPersist` keeps its buffer on failure and retries only when a later `hcPersist` reschedules the timer. If the owner says nothing else, the batch dies with the tab | a flush-failure counter, or a retry that does not depend on him typing again | **open** |
