...
| `api/notify.js` + `vercel.json` `/api/notify` | Booking notification emails to owner AND customer | **RESTORED — NOT DEAD** | Deleted 2026-08-17 on a grep showing zero callers. It was LIVE: an owner booking email went out 25 minutes before the deletion. The caller is not a literal `/api/notify` string in this repo, so the search was wrong, not the endpoint. Restored same night | Owners stop receiving booking emails. It still renders `booking.notes` raw, which is a real leak — but the fix is stripping markers, not deleting a live path |
