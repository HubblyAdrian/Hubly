...
| `api/notify.js` + `vercel.json` `/api/notify` | Booking notification emails to owner AND customer | **DELETED 2026-08-17** | Zero callers anywhere in the repo (only the vercel route referenced it). Rendered `booking.notes` RAW, so it leaked every internal marker — `[SMS_CONSENT:yes]`, `[RETURNING:yes]`, `[PAYNOW:$…]`, `[DISCOUNT:…]`, `[RPJOB:…]` — into an owner-facing email, and sent a second "You're booked!" to the customer | None. Dormant duplicate of the live notification path; the risk was someone wiring it up later and reintroducing the leak |
