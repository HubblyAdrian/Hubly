# Milestone 2 · Epic 8 — Business Home

**Status:** PASS  
**Checked:** 2026-07-24T16:05:12.327Z  
**Gate:** `npm run check:m2-epic8`

> Personalized business briefing — not a dashboard.  
> **30-Second Rule:** today · attention · highest-impact action · health · Ask Hubly

## Proven

| Requirement | Status |
|-------------|--------|
| Business Home (not dashboard) | ✅ |
| Morning Brief | ✅ |
| Today's Focus (one highest-impact) | ✅ |
| Business Health explainable | ✅ |
| Live Website card | ✅ |
| Industry-specific Today's Business | ✅ |
| Ask Hubly always docked | ✅ |
| Business Timeline | ✅ |
| Growth Opportunities + impact | ✅ |
| Workspace Intelligence | ✅ |
| Business Snapshot | ✅ |
| Adaptive empty / stage states | ✅ |
| 30-Second Rule | ✅ |
| Founder acceptance tests | ✅ |

## Morning Brief sample

> Good morning, Adrian  
> Last night I reviewed your business.  
> Here's what I'd focus on today.

## Today's Focus (pressure_washing)

> Upload five completed job photos.  
> Expected Impact: +18% customer trust.

## Stop

Do **not** begin Epic 9 until Founder Approval.
